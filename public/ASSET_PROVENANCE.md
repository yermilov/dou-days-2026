...
The asset was embedded in the template as an `<image>` element referencing an
in-memory `blob:` URL. It was captured by running a small in-page script (via
Chrome DevTools) that calls `fetch()` on the blob URL, converts the response
to a base64 data URL, and triggers a local PNG download.
